...
class TrainPipeline:
    ...
    def start_data_validation(self, data_ingestion_artifact: DataIngestionArtifact) -> DataValidationArtifact:
        try:
            ...
            validator = DataValidator(self.data_validation_config)
            validation_artifact = validator.run_validation(data_ingestion_artifact.data_stream)
    ...
    def run_pipeline(self) -> None:
        try:
            ...
            # 🛠 Optional: Debug stream safely using tee()
            stream_debug, stream_real = tee(data_ingestion_artifact.data_stream)
            sample = list(islice(stream_debug, 2))
            logging.info(f"🔍 Sample from ingested stream:\n{sample}")
            data_ingestion_artifact.data_stream = stream_real  # Reset generator
